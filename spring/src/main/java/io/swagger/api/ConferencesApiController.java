...
public class ConferencesApiController implements ConferencesApi {
    ...
    public ResponseEntity<String> conferencesViewconference(@ApiParam(value = "The unique identifier of each conference resource", required=true) @RequestPart(value="ConferenceSid", required=true)  String conferenceSid) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("")) {
            try {
                return new ResponseEntity<String>(objectMapper.readValue("", String.class), HttpStatus.NOT_IMPLEMENTED);
